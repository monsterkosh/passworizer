const { characters } = require('../Constants/CharactersConstants');

async function PassworizerAsync(difficulty) {
    ...
    const getOneRandomChar = async (string) => {
    ...
    const getVariousRandomChars = async (times, type) => {
        let result = [];
        if (typeof times !== 'number')
            throw new Error('Error: Type of times is not number');
        for (let index = 0; index < times; index++) {
            const element = await getOneRandomChar(type);
            result.push(element);
        }
    ...
    const shuffleArray = async (array) => {
        return await array.sort((a, b) => 0.5 - Math.random());
    };

    // Generates the final password
    // parameters should be (numbers) the amount of chars desired for each type
    const generatePassword = async (lows, uppers, numbers, symbols) => {
        // Get the individual results of each type
        const lowCharsResult = await getVariousRandomChars(lows, characters.lowChars);
        const firstUpperCharsResult = await getVariousRandomChars(
            1,
            characters.upperChars
        );
        const upperCharsResult = await getVariousRandomChars(
            uppers - 1,
            characters.upperChars
        );
        const numbersResult = await getVariousRandomChars(numbers, characters.numbers);
        const symbolsResult = await getVariousRandomChars(symbols, characters.symbols);
        ...
        const shuffledResult = await shuffleArray(temporaryResult);
...
module.exports = { PassworizerAsync }